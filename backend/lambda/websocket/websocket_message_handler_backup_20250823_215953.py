    ...
    try:
        ...
        if action == 'sendMessage':
            ...
            response = process_chat_message_with_bedrock(
                user_message, engine_type, connection_id, apigateway_client, conversation_history
            )
            return response
        else:
            # 알 수 없는 액션
...
def process_chat_message_with_bedrock(user_message, engine_type, connection_id, apigateway_client, conversation_history=None):
...
def stream_claude_response_inline(user_message, system_prompt, conversation_history=None):
    """Claude 4.0 스트리밍 응답 생성 (대화 히스토리 포함)"""
    try:
        ...
        if conversation_history:
            ...
            for msg in conversation_history:
                ...
                # Claude API는 'user'와 'assistant' role만 인식
        ...
        logger.info(f"Total messages to Claude: {len(messages)}")
        if messages:
            logger.info(f"First message: {messages[0]['role']}: {messages[0]['content'][:50]}...")
