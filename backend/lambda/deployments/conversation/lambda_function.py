...
def lambda_handler(event, context):
    ...
    try:
        ...
        elif '/conversations/' in path and method == 'PATCH':
            # 대화 제목 업데이트
            conversation_id = path.split('/conversations/')[-1]
            body = json.loads(event.get('body', '{}'))
            return update_conversation_title(conversation_id, body)
...
def update_conversation_title(conversation_id, body):
    """대화 제목 업데이트"""
    try:
        user_id = body.get('userId')
        new_title = body.get('title')
        
        logger.info(f"Updating title for conversation: {conversation_id}, userId: {user_id}")
        
        if not user_id or not new_title:
            return {
                'statusCode': 400,
                'headers': CORS_HEADERS,
                'body': json.dumps({'error': 'userId and title are required'})
            }
        ...
        # Decimal 변환 - 이미 정의된 전역 함수 사용
        updated_conversation = decimal_to_float(updated_conversation)
