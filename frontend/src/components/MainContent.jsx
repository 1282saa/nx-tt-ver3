...
import { fetchUsageFromServer } from '../services/usageService';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const percentage = await usageService.getUsagePercentage(selectedEngine, true); // 초기 로드는 강제 새로고침
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-text-300">
                    {selectedEngine} 엔진의 프로젝트 설명을 수정합니다
                  </p>
                </div>
                <button
